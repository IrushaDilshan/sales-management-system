...
export default function TransferItemsScreen() {
    ...
    const [selectedItems, setSelectedItems] = useState<{ [key: number]: number }>({}); // itemId -> qty to transfer
    ...
    const handleSubmit = async () => {
        ...
        // Validate Quantity
        for (const tItem of itemsToTransfer) {
            const stockItem = items.find(i => i.id === tItem.id);
            if (!stockItem || tItem.qty > stockItem.qty) {
                Alert.alert('Error', `Insufficient stock for ${stockItem?.name || 'Item ' + tItem.id}`);
                return;
            }
        }
        ...
        setSubmitting(true);
        try {
            // Sequential transfer calls (simpler than building a batch rpc for now)
            for (const tItem of itemsToTransfer) {
                const { error } = await supabase.rpc('transfer_rep_to_shop', {
                    p_rep_id: currentRepId,
                    p_shop_id: shopId,
                    p_product_id: tItem.id,
                    p_quantity: tItem.qty,
                    p_notes: 'Rep Transfer App'
                });
    ...
    const renderItem = ({ item }: { item: StockItem }) => {
        const transferQty = selectedItems[item.id] || 0;
            ...
            <View style={styles.card}>
                ...
                <View style={styles.inputContainer}>
                    <TouchableOpacity
                        style={styles.qtyBtn}
                        onPress={() => {
                            const newQty = Math.max(0, transferQty - 1);
                    ...
                    <TextInput
                        ...
                        value={transferQty > 0 ? transferQty.toString() : ''}
                        placeholder="0"
                    ...
                    <TouchableOpacity
                        style={styles.qtyBtn}
                        onPress={() => {
                            const newQty = Math.min(item.qty, transferQty + 1);
                            handleQtyChange(item.id, newQty.toString());
                        }}
                    >
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <View style={styles.header}>
                ...
                <View>
                    ...
                    <Text style={styles.subtitle}>To: {shopName}</Text>
                </View>
                <View style={{ width: 40 }} />
            </View>
